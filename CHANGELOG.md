...
- **Asta desktop — colonna "Chiamata" collassabile**: un bottone nella testata
  del listone giocatori (colonna sinistra) lo riduce a una fascia stretta
  (48px), nascondendo ricerca, filtro ruolo, ordinamento e lista per lasciare
  più spazio alla colonna centrale del giocatore in asta. Un secondo click
  riespande la colonna. Solo vista desktop: la vista telefono usa già le tab
  (`lista`/`alternative`/`log`) per lo stesso scopo.
...
- **Asta — nota di scouting del giocatore in chiamata**: la nota personale della
  valutazione (`selectedValuation.note`), se non vuota, è mostrata sotto il
  riquadro del giocatore in asta sia in vista desktop sia telefono (nessun
  placeholder quando manca). Quando il prezzo digitato porta il verdetto in zona
  `over` (oltre il max bid / "Fuori mercato") la nota è evidenziata riusando il
  linguaggio visivo esistente (accent-2), senza introdurre nuovi colori. Nuovo
  campo `selectedNote` sul view di `AuctionMode` e nuovo componente condiviso
  `ScoutingNote`.
...
- **Valutazioni — l'import JSON può sovrascrivere le modifiche personali**: se
  l'utente ha override in quella lega, l'import di un JSON di valutazioni chiede
  conferma ("Hai N valutazioni modificate: sovrascriverle?"); confermando,
  `POST /leagues/:id/valuations/import?overwriteOverrides=1` azzera gli override
  personali dei giocatori presenti nel file dopo aver aggiornato la base
  condivisa. Il report mostra "Modifiche azzerate: N". Annullando, l'import
  resta non distruttivo come prima.
...
- **Import listone con soprannomi tra virgolette**: `parseCsvRows` ora usa
  `relax_quotes`, così i nomi completi del listone ufficiale Fantacalcio.it che
  contengono un soprannome tra virgolette in un campo non quotato (es.
  `Goncalves "Pote" Pedro`) non fanno più abortire l'intero import con
